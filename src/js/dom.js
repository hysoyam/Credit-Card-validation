...
export function createInput(labelText, params = {}) {
  ...
  let errorBlock = Crelt('div', { class: 'input-error-block hidden' }, params.ErrorText)
  let block = Crelt('div', { class: 'input-group' }, [label, input, errorBlock])
  ...
  function validation(e) {
    if (!input.value && e.type === 'blur' && e.target === input) { showError() }
...
export function createForm() {
  ...
  const paymentSystemIcon = Crelt('img', { class: 'payment-system-icon', id: 'payment-system-icon', src: stockIcon }, ['Icon'])
  ...
  const form = Crelt('form', { class: 'form' }, [cardNumber, expDate, CVV, email, confirmBtn])
...
function validationExpDate(value = '') {
  return CardValidator.expirationDate(value).isValid
}

function validationCardCVV(value) {
